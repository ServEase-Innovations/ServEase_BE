 ...
 * import org.springframework.stereotype.Service;
 * 
 * import com.springboot.app.constant.ServiceProviderConstants; // Import the
 * constants class
 * 
 * import com.springboot.app.dto.ServiceProviderDTO;
 * 
 ...
 * LoggerFactory.getLogger(ServiceProviderServiceImpl.class);
 * 
 * @Autowired
 * private SessionFactory sessionFactory;
 * 
 * @Autowired
 * private ServiceProviderMapper serviceProviderMapper; // Inject service
 * provider mapper
 * 
 * @Override
 * 
 ...
 * session.createQuery("from ServiceProvider", ServiceProvider.class)
 * .list();
 * 
 * return serviceProviders.stream()
 * .map(serviceProviderMapper::serviceProviderToDTO)
 ...
 * if (serviceProvider == null) {
 * throw new
 * RuntimeException(ServiceProviderConstants.SERVICE_PROVIDER_NOT_FOUND + id);
 * }
 * 
 * return serviceProviderMapper.serviceProviderToDTO(serviceProvider);
 * }
 ...
 * 
 * @Transactional
 * public List<ServiceProviderDTO> getfilters(LanguageKnown language, Double
 * rating, Gender gender,
 ...
 * 
 * // Create the query from the dynamically built HQL
 * Query<ServiceProvider> query = session.createQuery(hql.toString(),
 * ServiceProvider.class);
 * 
 * // Set parameters if they are not null
 * if (language != null) {
 ...
 * List<ServiceProvider> serviceProviders = query.getResultList();
 * 
 * // Convert the list of ServiceProvider entities to a list of
 * ServiceProviderDTOs
 * return serviceProviders.stream()
 * .map(serviceProviderMapper::serviceProviderToDTO)
 * .collect(Collectors.toList());
